...
SINGLE_BATTLE_TEST("Using Honey from bag heals 25% of target's max HP")
{
    GIVEN {
        ...
        TURN { USE_ITEM(player, ITEM_HONEY, partyIndex: 0); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Using Honey from bag caps at max HP")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Ripen does not double Honey's effect when used from bag")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { MaxHP(100); HP(50); }
        OPPONENT(SPECIES_WOBBUFFET);
